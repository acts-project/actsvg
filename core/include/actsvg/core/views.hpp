    ...
    namespace views
    {
        ...
        struct z_phi
        {
            ...
            contour operator()(const point3_container &vertices_) const
            {
                ...
                if (_protect_phi)
                {
                    auto min_phi = std::min_element(phi_values.begin(), phi_values.end());
                    auto max_phi = std::max_element(phi_values.begin(), phi_values.end());

                    if ((*min_phi) < 0. and (*max_phi) > 0. and ((*max_phi) - (*min_phi)) > M_PI)
                    {
                        for (auto &cv : c)
                        {
                            if (cv[1] < 0.)
