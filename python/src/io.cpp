...
void add_io_module(context& ctx) {
    ...
    // Adding a reader function from obj files for polygons
    {
        ...
        io.def(
            "polygons_from_obj", [](const std::vector<std::string>& objfiles,
                ...
                for (const auto& objn : objfiles) {
                    ...
                    if (file.is_open()) {
                        ...
                        for (auto [ip, pp] :
                             utils::enumerate(polygon_proxies)) {
    ...
    {
        io.def("steps_from_obj", [](const std::vector<std::string>& objfiles) {
            // The return polygon surfaces
    ...
    {
        // Write the file
        /// @todo add this as a function to the file
        io.def(
            "write",
            [](const svg::file& f, const std::string& fn) {
                std::ofstream fout;
                fout.open(fn);
                fout << f;
                fout.close();
